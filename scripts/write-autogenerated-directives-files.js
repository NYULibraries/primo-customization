...
// When this script was first written, there was only one repo, with the CDN
// serving from a top-level `cdn/` directory.  The CDN content directory was
// later moved into a separate repo.  `primo-customization-cdn` branches are not
// sync'ed to the branches of this repo, so it's not at the moment feasible to
// update that repo automatically (and of course the git operations would have
// to be automated also).
// If using setup where `primo-customization-cdn` is symlinked to the top-level
// `cdn/` directory, this script will write out the new blank HTML files there,
// and the changes can be checked in manually after the repo has been set to
// a state where it is ready for the changes (i.e. all remote updates have been
// fetched, the appropriate branch is checked out, etc.)
const CDN_ROOT =
    path.join( ROOT, 'cdn', 'primo-customization' );
const CUSTOM_PACKAGE_JS_FILE =
    path.join( ROOT, 'custom/00_common/js/05-autogenerated-custom-directives.js' );
...
if ( fs.existsSync( CDN_ROOT ) ) {
    const cdnViews = fs.readdirSync( CDN_ROOT, { withFileTypes : true } );
    cdnViews.forEach( cdnView => {
        if ( cdnView.isDirectory() ) {
            writeCdnHtmlFiles( CDN_ROOT, cdnView.name, ids );
        }
    } );
}
