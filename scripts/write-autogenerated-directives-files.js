...
const CUSTOM_ROOT = path.join( ROOT, 'custom' );
const CUSTOM_PACKAGE_JS_FILE = path.join( ROOT, 'custom/00_common/js/06-autogenerated-custom-directives.js' );
...
VIEW_FOLDERS.forEach( viewFolder => {
    writeViewFolderHtmlFiles( CUSTOM_ROOT, viewFolder, ids );
});
...
function writeViewFolderHtmlFiles( customRoot, view, ids ) {
    const htmlDir = path.join( customRoot, view, 'html' );
...
function writeCustomPackageJsFile( customPackageJsFile, ids ) {
    let js = `// ****************************************
// 06-autogenerated-custom-directives.js
// ****************************************
//
...
`;
    ...
    ids.forEach( id => {
        ...
        VIEW_FOLDERS.forEach( viewFolder => {
            // if ( fileFound ) return; // Skip further iterations if file is found

            try {
                const htmlFilePath = path.join( CUSTOM_ROOT, viewFolder, 'html', `${ id }.html` );
                htmlContent = fs.readFileSync( htmlFilePath, { encoding: 'utf8' } );
                // fileFound = true; // File found, no need to continue the loop
            } catch ( err ) {
                console.error(`Error reading HTML file for ${id} in ${viewFolder}: ${err}`);
            }
        });

        // if ( !fileFound ) {
        //     console.error(`HTML file for ${id} not found in any view folder.`);
        //     return; // Skip this id if file wasn't found
        // }


            // Escape backticks and special characters in HTML content.
